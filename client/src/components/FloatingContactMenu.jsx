...
const FloatingContactMenu = () => {
  ...
  return (
    <div className="fixed top-1/2 right-0 -translate-y-1/2 z-[999999] flex flex-col items-end pointer-events-none">
      ...
      <div 
        className={`flex flex-col items-end pointer-events-auto transition-all duration-[400ms] ease-[cubic-bezier(0.175,0.885,0.32,1.275)] origin-right ${
          isMenuOpen ? "transform translate-x-0 opacity-100 visible" : "transform translate-x-[100%] opacity-0 invisible"
        }`}
      >
        {/* Enquire Now Tab */}
        <div className="relative group block w-[50px]">
          ...
          {/* Sliding Form */}
          <div className="absolute right-[50px] top-0 transition-all duration-400 ease-in-out transform translate-x-[20px] opacity-0 invisible group-hover:translate-x-0 group-hover:opacity-100 group-hover:visible focus-within:translate-x-0 focus-within:opacity-100 focus-within:visible w-[320px] bg-white shadow-[-5px_5px_20px_rgba(0,0,0,0.2)] rounded-l-[5px] pointer-events-none group-hover:pointer-events-auto focus-within:pointer-events-auto z-10 flex flex-col">
            
            <div className="bg-white text-brand-600 text-[18px] font-semibold py-[15px] px-[20px] border-b border-[#eeeeee] rounded-tl-[5px]">
              Write us!, Let's Talk!
            </div>
