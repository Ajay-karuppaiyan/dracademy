  ...
  return (
    <div className="flex h-screen flex-col overflow-hidden bg-white selection:bg-brand-100 selection:text-brand-900">
      ...
      <div className="flex flex-1 overflow-hidden">
              ...
              <div className="bg-white p-10">
                ...
                <div className="flex items-center justify-between pt-10 border-t border-slate-100">
                  <button
                    ...
                    disabled={isFirst}
                    className={`flex items-center gap-3 px-8 py-5 rounded-2xl font-black text-sm transition-all ${
                      isFirst ? "opacity-20 translate-x-4 pointer-events-none" : "hover:bg-slate-100 text-slate-600"
                    }`}
                  >
        ...
        <aside className={`${sidebarOpen ? "translate-x-0 w-full md:w-[450px]" : "translate-x-full w-0"} absolute md:relative right-0 top-0 h-full bg-white border-l border-slate-100 transition-all duration-500 z-40 flex flex-col shadow-2xl md:shadow-none overflow-hidden`}>
           <div className="p-8 border-b border-slate-50 flex items-center justify-between shrink-0">
             <div>
                <h3 className="text-2xl font-black text-slate-900 mb-1">Curriculum</h3>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{course.lessons?.length} Lessons in this path</p>
             </div>
           ...
           <div className="flex-1 overflow-y-auto no-scrollbar p-6 space-y-10">
              {(course.syllabus?.length ? course.syllabus : [{ topic: "Complete Curriculum" }]).map((topic, topicIdx) => {
                ...
                return (
                  <div key={topicIdx} className="space-y-4">
                     <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] line-clamp-1">
                          {topic.topic || `Module ${topicIdx + 1}`}
                        </h4>
                     </div>

                     <div className="space-y-3 relative pl-4 border-l-2 border-slate-100 ml-4">
                        {topicLessons.map((lesson) => {
                          ...
                          return (
                            <button
                              ...
                              disabled={isLocked}
                              className={`group w-full relative -ml-[18px] flex items-center gap-4 p-4 rounded-3xl transition-all ${
                                isActive 
                                  ? "bg-white shadow-[0_20px_40px_-15px_rgba(37,99,235,0.2)] ring-1 ring-brand-100 scale-[1.02] z-10" 
                                  : isLocked 
                                    ? "opacity-50 grayscale cursor-not-allowed" 
                                    : "hover:bg-slate-50 grayscale-0"
                              }`}
                            >
                               {/* Connection Dot */}
                               <div className={`absolute left-0 w-3 h-3 rounded-full border-4 border-white ring-2 translate-x-[1px] transition-colors ${
                                  isActive ? "bg-brand-600 ring-brand-200" : isFinished ? "bg-brand-500 ring-brand-50" : "bg-slate-200 ring-slate-100"
                               }`} />

                               <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl transition-all ${
                                  isActive ? "bg-brand-600 text-white shadow-lg shadow-brand-600/30" : isFinished ? "bg-brand-100 text-brand-600" : "bg-slate-100 text-slate-400"
                               }`}>
                                  {isFinished ? <CheckCircle2 size={22} strokeWidth={2.5} /> : lesson.type === "video" ? <PlayCircle size={22} /> : <FileText size={22} />}
                               </div>

                               <div className="flex-1 text-left min-w-0">
                                  <p className={`text-[12px] font-black leading-tight truncate ${isActive ? "text-slate-900" : "text-slate-500"}`}>
                                    {lesson.title}
                                  </p>
                                  <div className="mt-1 flex items-center gap-2">
                                     <span className="text-[9px] font-black uppercase text-slate-400 tracking-tighter">{lesson.duration || "5m"} </span>
                                     <span className="h-1 w-1 bg-slate-200 rounded-full" />
                                     <span className="text-[9px] font-black uppercase text-brand-500 tracking-tighter">{lesson.type}</span>
                                  </div>
                               </div>

                               {isLocked && <Lock size={14} className="text-slate-400 mr-2" />}
                               {isActive && <div className="h-2 w-2 rounded-full bg-brand-600 animate-ping absolute top-4 right-4" />}
                            </button>
                          );
                        })}
                     </div>
           ...
           {/* Feedback Trigger at bottom of sidebar if done */}
           {isCompleted && !certificate && (
              <div className="p-8 border-t border-slate-100 bg-slate-50/50">
                 <button 
                  onClick={() => setShowFeedbackModal(true)}
                  className="w-full flex items-center justify-center gap-3 bg-white border border-slate-200 p-4 rounded-2xl font-black text-xs text-slate-600 hover:shadow-md transition-all active:scale-95"
                 >
                   <Star size={18} className="text-brand-500" /> Share Your Experience
                 </button>
              </div>
           )}
        </aside>
                       ...
                       <div className="flex justify-center gap-4">
                          {[1, 2, 3, 4, 5].map((star) => (
                             <button
                                ...
                                className={`h-16 w-16 rounded-2xl transition-all duration-300 group flex items-center justify-center ${
                                   feedbackRating >= star ? "bg-brand-600 text-white shadow-xl shadow-brand-600/30 -translate-y-1" : "bg-slate-50 text-slate-200 hover:bg-slate-100"
                                }`}
                             >
